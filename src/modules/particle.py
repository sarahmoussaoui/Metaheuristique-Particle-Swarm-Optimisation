...
class Particle:
    ...
    def get_next_seed(self):
        ...
        if self.seed is None:
            return None
        self.seed_counter += 1
        return self.seed + self.seed_counter
    ...
    def initialize_velocity(self):
        ...
        if valid_swaps:
            ...
            self.velocity = rng.sample(valid_swaps, num_swaps)
    ...
    def update_velocity(
        ...
        for swap in self.velocity:
            if rng.random() < w and len(new_velocity) < self.max_velocity_size:
                i, j = swap
                if can_add_swap(i, j):
        ...
        if diff_indices:
            for i in diff_indices:
                r1 = rng.random()  # Use the operation-specific RNG
                if r1 < c1:
                    try:
        ...
        if diff_indices:
            for i in diff_indices:
                r2 = rng.random()  # Use the operation-specific RNG
                if r2 < c2:
                    try:
        ...
        rng = random.Random(self.get_next_seed())
        if rng.random() < mutation_rate and len(new_velocity) < self.max_velocity_size:
            attempts = 0
            while (
                attempts < MAX_ATTEMPTS_MUTATION
                and len(new_velocity) < self.max_velocity_size
            ):
                i, j = rng.sample(range(len(self.position)), 2)
                if (
                    can_add_swap(i, j)
    ...
    def apply_mutation(self, mutation_rate=0.1):
        ...
        if rng.random() < mutation_rate:
            ...
            rng = random.Random(self.get_next_seed())
            if rng.random() < 0.5:
                block_size = rng.randint(2, min(10, len(self.position) // 3))
                start = rng.randint(0, len(self.position) - block_size)
